...
class EAGLEWorker(TpModelWorker):
    ...
    def _create_decode_backend(self):
        """Create decode attention backend based on decode_attention_backend setting"""
        self.server_args.decode_attention_backend = self.server_args.attention_backend if self.server_args.decode_attention_backend is None else self.server_args.decode_attention_backend
        ...
        if backend_type not in backend_map:
            raise ValueError(f"EAGLE is not supported in decode attention backend {backend_type}")
        
        return backend_map[backend_type]()

    def _create_draft_extend_backend(self):
        """Create extend attention backend based on prefill_attention_backend setting"""
        self.server_args.prefill_attention_backend = self.server_args.attention_backend if self.server_args.prefill_attention_backend is None else self.server_args.prefill_attention_backend
        ...
        if backend_type not in backend_map:
            raise ValueError(f"EAGLE is not supported in prefill attention backend {backend_type}")
        
        return backend_map[backend_type]()

    def _create_flashinfer_decode_backend(self):
        if not global_server_args_dict["use_mla_backend"]:
            from sglang.srt.layers.attention.flashinfer_backend import FlashInferMultiStepDraftBackend
            self.has_prefill_wrapper_verify = True
            return FlashInferMultiStepDraftBackend(
                self.draft_model_runner, self.topk, self.speculative_num_steps
            )
        else:
            from sglang.srt.layers.attention.flashinfer_mla_backend import FlashInferMLAMultiStepDraftBackend
            self.has_prefill_wrapper_verify = True
    ...
    def _create_fa3_decode_backend(self):
        from sglang.srt.layers.attention.flashattention_backend import FlashAttentionMultiStepBackend
        return FlashAttentionMultiStepBackend(
            self.draft_model_runner, self.topk, self.speculative_num_steps
        )

    def _create_flashmla_decode_backend(self):
        from sglang.srt.layers.attention.flashmla_backend import FlashMLAMultiStepDraftBackend
        return FlashMLAMultiStepDraftBackend(
            self.draft_model_runner, self.topk, self.speculative_num_steps
        )

    def _create_trtllm_mha_decode_backend(self):
        from sglang.srt.layers.attention.trtllm_mha_backend import TRTLLMHAAttnMultiStepDraftBackend
        self.has_prefill_wrapper_verify = True
